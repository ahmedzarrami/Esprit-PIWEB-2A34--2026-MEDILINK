...
<section class="section-block narrow">
    ...
    <?php if (empty($medicaments)): ?>
        <div class="empty-state">
            ...
            <p>Essayez avec un autre mot-clé ou changez le tri.</p>
        </div>
    <?php else: ?>
        <div class="medicine-list-grid">
            <?php foreach ($medicaments as $medicament): ?>
                <article class="medicine-list-card">
                    <div class="top-row">
                        <span class="pill"><?= htmlspecialchars((string) ($medicament['forme'] ?? '')) ?></span>
                        <span class="price"><?= number_format((float) $medicament['prix'], 2, ',', ' ') ?> TND</span>
                    </div>
                    <h3><?= htmlspecialchars($medicament['nom']) ?></h3>
                    <p class="meta"><?= htmlspecialchars((string) ($medicament['dosage'] ?? '')) ?> - <?= htmlspecialchars((string) ($medicament['fabricant'] ?? '')) ?></p>
                    <p class="description"><?= htmlspecialchars(mb_strimwidth((string) ($medicament['description'] ?? ''), 0, 140, '...')) ?></p>
                    <div class="bottom-row">
                        <span>Stock : <?= (int) ($medicament['stock'] ?? 0) ?></span>
        ...
        <?php if ($totalPages > 1): ?>
            <nav class="pagination">
                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <a href="index.php?action=medicaments&search=<?= urlencode($search) ?>&sort=<?= urlencode($sort) ?>&page=<?= $i ?>"
